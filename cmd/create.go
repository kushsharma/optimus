...
func AskTaskSurveyQuestion(ques models.PluginQuestion, cliMod models.CommandLineMod) (models.PluginAnswers, error) {
	...
	for _, subQues := range ques.SubQuestions {
		if responseStr == subQues.IfValue {
			for _, subQ := range subQues.Questions {
				subQuesAnswer, err := AskTaskSurveyQuestion(subQ, cliMod)
				if err != nil {
					return nil, err
				}
