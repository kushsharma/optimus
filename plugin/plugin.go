...
func Initialize(pluginLogger hclog.Logger) error {
	...
	var pluginMap = map[string]plugin.Plugin{
		...
		models.ModTypeCLI:                cli.NewPluginClient(pluginLogger),
		models.ModTypeDependencyResolver: dependencyresolver.NewPluginClient(pluginLogger),
	}
	...
	for _, pluginPath := range discoveredPlugins {
		...
		// try requesting plugin with cli mod
		if rawMod, err := rpcClient.Dispense(models.ModTypeCLI); err == nil {
			if c, ok := rawMod.(models.CommandLineMod); ok {
				cliClient = c
				pluginLogger.Debug(fmt.Sprintf("%s mod found for: %s", models.ModTypeCLI, baseInfo.Name))
			}
		}

		// try requesting plugin with dependency resolver mod
		if rawMod, err := rpcClient.Dispense(models.ModTypeDependencyResolver); err == nil {
