...
func TestProjectJobRepository(t *testing.T) {
	...
	pluginRepo := new(mock.SupportedPluginRepo)
	pluginRepo.On("GetByName", gTask).Return(execUnit1, nil)
	pluginRepo.On("GetByName", tTask).Return(execUnit2, nil)
	pluginRepo.On("GetByName", gHook).Return(hookUnit1, nil)
	pluginRepo.On("GetByName", tHook).Return(hookUnit2, nil)
	adapter := NewAdapter(pluginRepo)
